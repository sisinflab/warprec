...
Ray Tune restarts the reported history of a resumed trial, including its `training_iteration`, its `result.json` and its `progress.csv`. WarpRec therefore selects the best checkpoint, and counts its training iterations in epochs, over the checkpoints of the whole trial, those written before the pause included. For the same reason the pruning schedulers measure time in `completed_epochs`, which carries on across a resume, and the `asha` scheduler keeps the scores it recorded at its rungs.
