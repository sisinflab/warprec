...
2. **Data Engine** — Applies configurable filtering and splitting strategies, including cold-start protocols, to produce clean, leak-free train/validation/test sets.
...
4. **Evaluation Engine** — Computes 44 GPU-accelerated metrics in a single pass with automated statistical significance testing, optional re-ranking, and inverse-propensity estimators that correct for exposure bias.
...
- **78 Built-in Algorithms**: WarpRec ships with 78 state-of-the-art recommendation models spanning 8 paradigms — Unpersonalized, Content-Based, Collaborative Filtering (e.g., `LightGCN`, `EASE`$^R$, `MultiVAE`), Context-Aware (e.g., `DeepFM`, `xDeepFM`), Sequential (e.g., `SASRec`, `BERT4Rec`, `GRU4Rec`), Knowledge-Aware (e.g., `CKE`, `KGAT`), Multimodal (e.g., `VBPR`, `FREEDOM`), and Hybrid. All models are fully configurable and extend a standardized base class, making it easy to prototype custom architectures within the same pipeline.
...
- **44 GPU-Accelerated Metrics**: The evaluation suite covers 44 metrics across 8 families — Accuracy, Rating, Coverage, Novelty, Diversity, Bias, Fairness, and Debiased — including multi-objective metrics for simultaneous optimization of competing goals, and inverse-propensity estimators that correct for exposure bias. All metrics are computed with full GPU acceleration for large-scale experiments.
