...
class EFD(TopKMetric):
    ...
    Attributes:
        free_discovery (Tensor): Number of valid (unseen) recommendations in the top-k.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.add_state(
            "free_discovery", default=torch.tensor(0.0), dist_reduce_fx="sum"
        )
    ...
    def update(self, preds: Tensor, target: Tensor):
        ...
        # Update
        self.free_discovery += self.dcg(rel * novelty).sum()
        self.users += target.shape[0]

    def compute(self):
        """Computes the final value of the metric."""
        return (
            self.free_discovery / (self.users * self.discounted_sum(self.k))
            if self.users > 0
