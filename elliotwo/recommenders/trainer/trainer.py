...
class Trainer:
    ...
    Args:
        model_class (Type[AbstractRecommender]): The model to optimize.
        ...
        metric (Type[AbstractMetric]): The metric to use as validation.
    ...
    def train_and_evaluate(self) -> Tuple[AbstractRecommender, dict]:
        ...
        """
        logger.separator()
        logger.msg(f"Starting hyperparameter tuning for {self._model_class.__name__}")
        ...
        logger.positive(
            f"Hyperparameter tuning for {self._model_class.__name__} ended successfully."
        )
        logger.separator()
    ...
    def _objective_function(
        ...
        model = self._model_class(config, dataset, params)
        model.fit()
