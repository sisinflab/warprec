...
@model_registry.register(name="gSASRec_gts")
class gSASRec_gts(IterativeRecommender, SequentialRecommenderUtils):
    """gSASRec with GBCE loss - legacy compatible implementation."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.output_embedding = torch.nn.Embedding(
            self.n_items + 2, self.embedding_size
        )
    ...
    def sasrec_forward(self, input_seq: Tensor) -> tuple[Tensor, list]:
        ...
        seq = self.item_embedding(input_seq)
    ...
    def get_logits(self, model_out: Tensor) -> Tensor:
        """Calcola i logits per i primi n_items (escludendo padding tokens)."""
        return torch.matmul(model_out, self.output_embedding.weight[:self.n_items].t())
    ...
    def custom_train_loop(
        ...
        """Training loop with GBCE loss and device switching."""
        ...
        for epoch in range(self.epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                batch = {key: value.to(self.train_device) for key, value in batch.items()}
                optimizer.zero_grad()
                ...
                pos_neg_concat[pos_neg_concat < 0] = self.padding_token_id
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        last_logits = self.get_logits(hidden[:, -1, :])
    ...
    def recommend_impl(self, batch: dict[str, Tensor], limit: int, mode: str):
        ...
        if mode == "val":
            ...
            all_logits_for_loss = self.get_logits(last_hidden_state[:, -2, :])
            pos_neg_concat = torch.cat(
                [labels.unsqueeze(-1), negatives], dim=-1
            )
            # Clip indices to valid range for gather
            ...
            logits_for_ndcg = self.get_logits(last_hidden_state[retain_idxs, -1, :])
