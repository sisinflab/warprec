...
The `Proxy Recommender` is a versatile and powerful tool designed to evaluate recommendation outputs generated by external frameworks. It is intended for **local usage only** and is not compatible with the **Ray-based training module**. Due to this limitation, it must be executed within a **custom pipeline**. You can either run the sample code provided [here](../../sample_pipelines/proxy_evaluation.py), or develop your own evaluation script by directly leveraging the ProxyRecommender model. This flexibility allows you to seamlessly integrate third-party recommendation results into the evaluation pipeline of the framework.
