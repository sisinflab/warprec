...
The `Metrics` module offers a collection of state-of-the-art metrics for evaluating recommender systems. Users can easily configure and access these metrics via configuration file. To optimize memory utilization and ensure high-speed computation, WarpRec performs metric calculations in batches, leveraging [PyTorch](https://pytorch.org/) tensors for efficient processing.

For a detailed exposition and a complete list of **out of the box** metrics, refer to the [metrics documentation](metrics/README.md)

## 🧮 Evaluator

The `Evaluator` module is engineered to align seamlessly with WarpRec's metric structure, incorporating advanced optimizations for demanding evaluation tasks. The subsequent sections delineate the operational principles of the evaluation process.
...
While this structure may suffice for small datasets, its computational cost escalates considerably with an increasing number of users or relevant items per user. WarpRec addresses this limitation through two primary optimizations.
...
Within the WarpRec evaluation module, **Metric Blocks** are defined as computationally intensive operations that are commonly shared across multiple metrics. A prime example is the calculation of **relevant recommended items**. This specific task, if performed independently for each metric, would lead to significant computational overhead. WarpRec addresses this by computing such shared operations only **once per batch**, and then distributing the results to all relevant metrics. This approach significantly optimizes the overall evaluation process.
...
- And other foundational computations that underpin various evaluation metrics.
