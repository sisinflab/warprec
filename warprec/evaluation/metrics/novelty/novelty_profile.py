...
class NoveltyProfileMetric(UserAverageTopKMetric):
    """Shared behaviour of the metrics that discount a hit by item novelty.

    Both metrics in this family score a recommendation by how relevant it is,
    weighted by how novel the item is, and differ only in whether the novelty
    profile is built on a logarithmic discount.
    ...
    #: Whether the novelty profile discounts popularity logarithmically.
        ...
        **kwargs: Any,
        ...
        self.register_buffer(
            ...
            self.compute_novelty_profile(
                item_interactions, num_users, log_discount=self._LOG_DISCOUNT
            ),
        )
    ...
    def unpack_inputs(
        ...
        """Select the relevance blocks this metric was configured with.

        Args:
            preds (Tensor): The predictions of the model.
            **kwargs (Any): The computed metric blocks.

        Returns:
            Tuple[Tensor, Tensor, Tensor]: The relevance, the valid users and the
                top-k relevance.
        """
    ...
    def name(self):
        ...
        return f"{self.__class__.__name__}[{self.relevance}]"
