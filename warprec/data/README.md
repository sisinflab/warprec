...
When an experiment is started, WarpRec automatically initializes a dedicated directory for the experiment, unless this behavior is explicitly disabled via configuration. The directory follows the structure below:
...
All outputs generated by WarpRec are stored in this directory, each associated with a timestamp to ensure reproducibility and allow multiple runs within the same environment. Below is a description of the directory contents:

- **evaluation/**: Contains model evaluation results, including metric scores and a detailed report with temporal and spatial information (unless this output is disabled).
- **params/**: Stores hyperparameter configurations found during HPO phase.
- **recs/**: Contains the final recommendations produced by each model (if this option is enabled).
- **serialized/**: Stores the serialized (pickled) versions of trained models (if serialization is enabled).
- **split/**: Contains the dataset splits (train/validation/test) used for the experiment (if this option is enabled).
- **config.json**: A fully resolved configuration file for the experiment. This includes both user-specified values and parameters automatically set to default, allowing precise tracking of the configuration used for each run.

Each file and subdirectory is timestamped to facilitate running multiple experiments in the same workspace without overwriting results.

#### 🎯 Evaluation reporting

Within the `evaluation/` directory, WarpRec produces two primary artifacts:

1. `Overall_Results` – a tabular file summarizing evaluation metrics per model and cutoff `k`. Example layout:
...
- One row per `(model, k)` combination.
- Metrics reflect the configured evaluation protocol (e.g., full vs. sampled).
- The file is timestamped to avoid overwriting results across runs.

2. `Time_Report` – a timing and environment summary with wall-clock measurements. Fields include:

- **Data Preparation Time**: End-to-end data prep (splitting, filtering, evaluation sampling if enabled).
...
- **Inference Time (ms)**: Time to predict for 1,000 users × 1,000 items; intended as a comparative guideline.
- **Total Time**: Aggregate end-to-end duration for the model’s experiment pipeline.

Unless otherwise specified, durations are wall-clock; inference time is reported in milliseconds. File names include timestamps to support multiple runs in the same workspace.

#### ✂️ Split structure

The `split/` directory contains the dataset partitions generated by WarpRec, if dataset splitting is enabled. Its structure typically follows this pattern:
...
- At the **top level** of the `split/` directory, you will find the primary **train/test split**.
- In the example above, files are stored in **TSV format**, but the output format is configurable.
- If validation folds are generated, subdirectories named `1/`, `2/`, `3/`, ... are created, each containing its own **train/validation split**.

In a holdout or leave-k-out setup, there will typically be a single fold (e.g., `1/`). In a cross-validation setup, multiple folds will be present, with the number of folds determined by the configuration parameters.
