...
@model_registry.register(name="SASRecJPQ")
class SASRecJPQ(IterativeRecommender, SequentialRecommenderUtils):
    """Port di SASRecJPQ con Product Quantization per item embeddings."""
    ...
    def _assign_centroids(self, sessions: Sessions) -> None:
        ...
        print(f"\n[DEBUG] Assigning codes for {len(train_users)} users...")
        self.item_codes_layer.assign_codes(train_users)
        print(f"[DEBUG] Codes assigned! item_codes stats:")
        print(f"  > Shape: {self.item_codes_layer.item_codes.shape}")
        print(f"  > Unique values: {torch.unique(self.item_codes_layer.item_codes).numel()}")
        print(f"  > Min/Max: {self.item_codes_layer.item_codes.min().item()} / {self.item_codes_layer.item_codes.max().item()}")
        print(f"  > First 10 items codes:\n{self.item_codes_layer.item_codes[:10]}")
    ...
    def get_logits(self, model_out: Tensor) -> Tensor:
        """Calcola i logits usando JPQ scoring per i primi n_items."""
        # score_all_items already handles only valid items (0 to n_items-1)
    ...
    def train_step(self, batch: Any, *args: Any, **kwargs: Any) -> Tensor:
        """Training step with JPQ - matching TensorFlow original implementation."""
        seq = batch["seq"]
        labels = batch["labels"]
        negatives = batch["negatives"]
        ...
        # Loss computation with JPQ - EXACTLY as in TF original
        batch_size = seq.size(0)
        seq_len = seq.size(1)
        ...
        # Gather positive and negative logits
        ...
        # ORIGINAL TF LOSS - NOT BPR!
        # minus_positive_logprobs = softplus(-positive_logits)
        # minus_negative_logprobs = sum(softplus(negative_logits))
        # minus_average_logprobs = (minus_positive_logprobs + minus_negative_logprobs) / (num_negatives + 1)
        minus_positive_logprobs = torch.nn.functional.softplus(-positive_logits)
        minus_negative_logprobs = torch.nn.functional.softplus(negative_logits)  # Only 1 negative per position
        minus_average_logprobs = (minus_positive_logprobs + minus_negative_logprobs) / 2.0  # (1 + 1) negatives
