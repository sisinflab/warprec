...
|---|---|
| `grid` | Exhaustive grid search over all hyperparameter combinations. |
| `random` | Random sampling from the search space. |
| `hopt` | Bayesian optimization via HyperOpt (Tree-structured Parzen Estimators). |
| `optuna` | Bayesian optimization via Optuna with advanced pruning. |
| `bohb` | Bayesian Optimization and HyperBand for combined exploration and early stopping. |
