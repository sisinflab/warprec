...
models:
  ProxyRecommender:
    path: /path/to/recommendations.tsv
```

The `path` parameter points to the file containing the precomputed recommendation lists. WarpRec will load these recommendations and pass them through the configured evaluation pipeline, producing the same output artifacts (evaluation reports, statistical tests, etc.) as for any built-in model.
